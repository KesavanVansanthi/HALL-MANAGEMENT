...
public class CustomerDetailsDatabase {
	...
	public void view(String username,String pass) throws SQLException {
			...
			ResultSet result = stat.executeQuery("SELECT USER_NAME, FIRST_NAME, LAST_NAME, AGE, DATE_OF_BIRTH, CITY, PHONE_NO, EMAIL, PASSWORD, CATEGORY FROM HALL.USERS WHERE USER_NAME='"+username+"' AND PASSWORD='"+pass+"'");
			
			while(result.next()) {
				System.out.println("\n             *******************************");
				System.out.println("             *   "+Colors.LAVENDER+"--- CUSTOMER DETAILS ---"+Colors.RESET+"  *");
				System.out.println("             *******************************");
